...
class RetNetRelPos(nn.Module):
    def __init__(self, args):
        ...
        decay_init = torch.log(1 - 2 ** (-5 - torch.arange(args.decoder_retention_heads, dtype=torch.float)))
        self.register_buffer("angle", angle)

        # ATTEMPTED FIX: Tried making decay learnable, but backward pass produces NaN gradients
        # The retention mechanism's math is not compatible with learnable decay in this implementation
        # KEEPING as buffer (fixed) to maintain numerical stability
        self.register_buffer("decay", decay_init.clone())
